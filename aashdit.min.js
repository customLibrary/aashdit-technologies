...
function renderCalendar(viewDate) {
  ...
  for (let d=1; d<=daysInMonth; d++) {
    ...
    day.onmouseover = () => {
      if (!day.classList.contains("disabled")) day.style.background = "#e0f0ff";
    };
    day.onmouseout = () => {
...
function showCalendar(customDate = null) {
  ...
  document.addEventListener("click", function(e) {
    if (!calendarModal.contains(e.target) && e.target !== startInput && e.target !== endInput) {
      calendarModal.style.display = "none";
    }
  });
}
...
// toastify js 
function showToast(message, type="info", duration=4000, position="top-right") {
...
const AashBox = (() => {
  function createModal(options) {
    document.querySelectorAll(".aash-modal-backdrop").forEach(e => e.remove());
    ...
    modal.innerHTML = `
      <div class="aash-modal-dialog ${options.centerVertical ? "aash-modal-centered" : ""}">
  ...
  function renderInput(field, idx) {
    ...
    let html = `<div class="aash-field"><label>${field.label || ""}</label><br/>`;

    if (field.inputType === "select") {
      html += `<select id="${inputId}" class="aash-input" ${field.required ? "required" : ""}>`;
      (field.inputOptions || []).forEach(opt => {
        html += `<option value="${opt.value}" ${opt.value === field.value ? "selected" : ""}>${opt.text}</option>`;
      });
      html += `</select>`;
    } 
      ...
      (field.inputOptions || []).forEach((opt, i) => {
        ...
        html += `
          <label>
            <input type="${field.inputType}" name="${inputId}" id="${id}" value="${opt.value}"
              ${field.value && (Array.isArray(field.value) ? field.value.includes(opt.value) : field.value === opt.value) ? "checked" : ""} 
      ...
      html += `<input type="${field.inputType || "text"}" id="${inputId}" value="${field.value || ""}" class="aash-input"
        ${field.required ? "required" : ""}
  ...
  return {
      ...
      createModal({
        ...
        buttons: opts.buttons || {
          ok: {
            ...
            className: "aash-btn aash-btn-sm aash-btn-contained aash-btn-success",
    ...
    confirm(opts) {
      createModal({
        ...
        buttons: opts.buttons || {
          cancel: { label: opts.cancelLabel || "Cancel", className: "aash-btn aash-btn-error aash-btn-sm aash-btn-contained me-2" },
          confirm: { label: opts.confirmLabel || "OK", className: "aash-btn aash-btn-success aash-btn-sm aash-btn-contained" }
        },
        callback: opts.callback
      });
    },
      ...
      createModal({
        ...
        buttons: {
          cancel: { 
            ...
            className: "aash-btn aash-btn-error aash-btn-sm aash-btn-contained me-2", 
          },
          confirm: { 
            label: opts.confirmLabel || "Submit", 
            className: "aash-btn aash-btn-success aash-btn-sm aash-btn-contained",
            callback: function () {
              let valid = true;
              ...
              inputs.forEach(input => {
                ...
                if (field.inputType === "checkbox") {
                  values[field.name || inputId] = [...document.querySelectorAll(`input[name='${inputId}']:checked`)].map(i => i.value);
                } 
                else if (field.inputType === "radio") {
                  const sel = document.querySelector(`input[name='${inputId}']:checked`);
